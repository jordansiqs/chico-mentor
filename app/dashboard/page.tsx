...
export default function ChicoDashboard() {
  ...
  const C = {
    ...
    shadowHov:  "0 4px 20px rgba(0,0,0,0.09)",
  };
  ...
  const troncoColor  = profile?.tronco === "românico" ? "#FF3B30" : C.blue;
  const troncoLabel  = profile?.tronco === "românico" ? "Tear Românico" : "Tear Germânico";
    ...
    <>
      ...
      <div style={{ display:"flex", height:"100vh", background:C.bg, fontFamily:"'Nunito', -apple-system, BlinkMacSystemFont, sans-serif", color:C.text, overflow:"hidden" }}>
        ...
        <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden", minWidth:0 }}>
          ...
          {/* Top bar */}
          <header style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 20px", height:TOPBAR_H, background:"rgba(255,255,255,0.92)", backdropFilter:"blur(20px)", borderBottom:`1px solid ${C.border}`, flexShrink:0, zIndex:100 }}>
            <div style={{ display:"flex", alignItems:"center", gap:"12px" }}>
              {isMobile && (
                <button onClick={()=>setSidebarOpen(v=>!v)} style={{ width:34, height:34, borderRadius:"8px", border:"none", background:"transparent", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
                ...
                <span style={{ fontSize:"22px", fontWeight:800, color:"#1A4A8A", letterSpacing:"-0.01em", fontFamily:"'Nunito', sans-serif" }}>chico</span>
              </div>
          ...
          <div style={{ flex:1, overflow:"hidden", display:"flex", flexDirection:"column" }}>
            ...
            {activeTab === "chat" && (
              <>
                {/* Barra ferramentas */}
                <div style={{ padding:"7px 20px", background:"rgba(255,255,255,0.85)", backdropFilter:"blur(12px)", borderBottom:`1px solid ${C.borderSub}`, display:"flex", alignItems:"center", gap:"8px", flexShrink:0 }}>
                  {!showRoteiroInput ? (
                    <>
                ...
                {/* Banner sugestão */}
                {sugestao && (
                  <div style={{ padding:"10px 20px", background:"linear-gradient(135deg,rgba(94,92,230,0.06),rgba(191,90,242,0.04))", borderBottom:`1px solid rgba(94,92,230,0.10)`, display:"flex", alignItems:"center", gap:"10px", flexShrink:0 }}>
                    <div style={{ flex:1 }}>
                      <div style={{ fontSize:"11px", fontWeight:700, color:"#5E5CE6", marginBottom:"2px", letterSpacing:"0.02em" }}>{sugestao.titulo}</div>
                      <div style={{ fontSize:"12px", color:C.textSub, lineHeight:1.4 }}>{sugestao.mensagem}</div>
                    </div>
                    <div style={{ display:"flex", gap:"6px", flexShrink:0 }}>
                      ...
                      <button onClick={()=>setSugestao(null)}
                        style={{ width:22, height:22, borderRadius:"50%", border:"none", background:C.bgInput, color:C.textDim, fontSize:"13px", cursor:"pointer" }}>×</button>
                    </div>
                  </div>
                )}
                ...
                {audio.isListening && (
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:"8px", padding:"6px", background:"rgba(255,59,48,0.05)", borderTop:`1px solid rgba(255,59,48,0.10)` }}>
                ...
                <div style={{ padding:isMobile?"8px 12px 12px":"10px 24px 14px", background:"rgba(255,255,255,0.92)", backdropFilter:"blur(20px)", borderTop:`1px solid ${C.border}`, flexShrink:0 }}>
                  ...
                  {!isMobile && <p style={{ margin:"4px 0 0", fontSize:"10px", color:C.textMuted, textAlign:"center" }}>Enter para enviar · Shift+Enter para nova linha</p>}
                </div>
              </>
            )}
        ...
        {!isMobile && (
          <nav style={{ width:NAV_W, display:"flex", flexDirection:"column", alignItems:"center", paddingTop:"12px", paddingBottom:"12px", background:C.bgPanel, borderLeft:`1px solid ${C.border}`, gap:"2px", flexShrink:0 }}>
            {navTabs.map(tab => {
              const active = activeTab === tab.id;
              return (
                <button key={tab.id} onClick={()=>setActiveTab(tab.id)} title={tab.label}
                  style={{ width:62, height:58, borderRadius:"14px", border:"none", cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:"4px", background:active?C.blueDim:"transparent", transition:"all 0.15s", position:"relative" as const }}>
                  ...
                  <span style={{ fontSize:"11px", fontWeight:active?700:500, color:active?"#1A4A8A":C.textMuted, letterSpacing:"0.01em" }}>{tab.label}</span>
                </button>
              );
            })}
            <div style={{ flex:1 }}/>
            ...
            <button onClick={()=>setSidebarOpen(v=>!v)} title="Biblioteca de Nexos"
              style={{ width:62, height:58, borderRadius:"14px", border:"none", cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:"4px", background:sidebarOpen?"#FFF3E0":"transparent", transition:"all 0.15s" }}>
              <Icon.Library c={sidebarOpen?C.blue:C.textMuted}/>
              <span style={{ fontSize:"11px", fontWeight:sidebarOpen?700:500, color:sidebarOpen?"#E07820":C.textMuted }}>Nexos</span>
            </button>
          </nav>
        )}
              ...
              {isFetchingCards
                ...
                : filteredCards.length===0
                  ? <div style={{ display:"flex", flexDirection:"column", alignItems:"center", padding:"36px 12px", textAlign:"center", gap:"10px" }}>
                      ...
                      <p style={{ margin:0, fontSize:"13px", color:C.textDim, lineHeight:1.5 }}>
        ...
        {/* ── MOBILE: sidebar desliza pela direita ── */}
        {isMobile && sidebarOpen && (
          <>
            <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.4)", zIndex:150 }} onClick={()=>setSidebarOpen(false)}/>
            <aside style={{ position:"fixed", top:0, right:0, bottom:0, width:"82%", maxWidth:320, zIndex:160, background:C.bgPanel, borderLeft:`1px solid ${C.border}`, display:"flex", flexDirection:"column", padding:"16px", boxShadow:"-4px 0 20px rgba(0,0,0,0.12)" }}>
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"14px" }}>
                <h2 style={{ margin:0, fontSize:"16px", fontWeight:700, color:C.text }}>Biblioteca</h2>
          ...
          <nav style={{ position:"fixed", bottom:0, left:0, right:0, height:MOBILE_NAV_H, background:"rgba(255,255,255,0.95)", backdropFilter:"blur(20px)", borderTop:`1px solid ${C.border}`, display:"flex", alignItems:"center", zIndex:200, paddingBottom:"env(safe-area-inset-bottom)" }}>
            {navTabs.map(tab=>{ const active=activeTab===tab.id; return (
              <button key={tab.id} onClick={()=>{setActiveTab(tab.id);setSidebarOpen(false);}}
                ...
                {tab.icon(active?C.blue:C.textMuted)}
                <span style={{ fontSize:"11px", fontWeight:active?700:500, color:active?"#1A4A8A":C.textMuted }}>{tab.label}</span>
              </button>
            );})}
